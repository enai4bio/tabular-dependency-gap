...
from ef_vfm.models.flow_model_fix import ExpVFMFix
...
def main(args):
    ...
    # 047 capacity check: allow an alternate config (e.g. width x4) via --config_path.
    # Default is unchanged, so 1x baselines keep using the original toml.
    config_path = getattr(args, 'config_path', None) or f'{curr_dir}/configs/ef_vfm_configs.toml'
    ...
    if args.deterministic:
        ...
        print(f"DETERMINISTIC MODE is enabled!!! seed={seed}")
    ...
    use_fix = getattr(args, 'fix', False)
    rank = getattr(args, 'rank', 2)
    mechanism = getattr(args, 'mechanism', 'p1p3')
    if use_fix:
        model = UniModMLPFix(**raw_config['unimodmlp_params'], rank=rank, mechanism=mechanism)
    else:
        model = UniModMLP(**raw_config['unimodmlp_params'])
    model.to(device)

    if use_fix:
        flow_model = ExpVFMFix(
