...
class TabMetrics(object):
    ...
    def evaluate_mle(self, syn_data, seed=42):
        ...
        if task_type == 'regression':
            best_r2_scores, best_rmse_scores = evaluator(train, test, info, val=val, seed=seed)
            ...
            best_f1_scores, best_weighted_scores, best_auroc_scores, best_acc_scores, best_avg_scores = evaluator(train, test, info, val=val, seed=seed)
        ...
        # Minority-class metrics (binary/multiclass only)
        if task_type in ('binclass', 'multiclass'):
            best = overall_scores['best_auroc_scores']['XGBClassifier']
            out_metrics['mle/minority_f1'] = best.get('binary_f1', float('nan'))
            out_metrics['mle/minority_recall'] = best.get('minority_recall', float('nan'))
            out_metrics['mle/minority_precision'] = best.get('minority_precision', float('nan'))
        out_extras = {
            "mle": overall_scores,
        }
        return out_metrics, out_extras
    
    def evaluate_per_class_density(self, syn_data):
        """Per-class shape and trend (SDV QualityReport run separately on each class subset)."""
        info = deepcopy(self.info)
        ...
        new_real, new_syn, metadata = reorder(real_data, syn_data, info)

        # After reorder: target is the LAST column (binclass/multiclass appends to cat)
        target_col = len(new_real.columns) - 1

        # Build metadata without target column
        meta_nolabel = deepcopy(metadata)
        new_cols = {}
        j = 0
        for i in sorted(meta_nolabel['columns'].keys()):
            if i == target_col:
                continue
            new_cols[j] = meta_nolabel['columns'][i]
            j += 1
        meta_nolabel['columns'] = new_cols

        classes = sorted(new_real[target_col].unique())
        real_counts = {c: int((new_real[target_col] == c).sum()) for c in classes}
        total = sum(real_counts.values())

        out_metrics = {}
        for c in classes:
            real_c = new_real[new_real[target_col] == c].drop(columns=[target_col]).reset_index(drop=True)
            syn_c  = new_syn[new_syn[target_col]  == c].drop(columns=[target_col]).reset_index(drop=True)
            real_c.columns = range(len(real_c.columns))
            syn_c.columns  = range(len(syn_c.columns))

            if len(real_c) < 5 or len(syn_c) < 5:
                out_metrics[f'per_class/class_{c}/shape'] = float('nan')
                out_metrics[f'per_class/class_{c}/trend'] = float('nan')
                continue
            try:
                import warnings
                with warnings.catch_warnings():
                    ...
                    qr = QualityReport()
                    qr.generate(real_c, syn_c, meta_nolabel, verbose=False)
                props = qr.get_properties()
                out_metrics[f'per_class/class_{c}/shape'] = float(props['Score'][0])
                out_metrics[f'per_class/class_{c}/trend'] = float(props['Score'][1])
            except Exception as e:
                print(f"[per_class_density] class {c} error: {e}")
                out_metrics[f'per_class/class_{c}/shape'] = float('nan')
                out_metrics[f'per_class/class_{c}/trend'] = float('nan')

        # Weighted mean across classes (by real class frequency)
        valid = [c for c in classes if not np.isnan(out_metrics.get(f'per_class/class_{c}/shape', float('nan')))]
        if valid:
            out_metrics['per_class/weighted_shape'] = sum(
                real_counts[c] / total * out_metrics[f'per_class/class_{c}/shape'] for c in valid)
            out_metrics['per_class/weighted_trend'] = sum(
                real_counts[c] / total * out_metrics[f'per_class/class_{c}/trend'] for c in valid)
