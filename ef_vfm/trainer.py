...
class Trainer:
    ...
    def sample_synthetic(self, num_samples, keep_nan_samples=True, ema=False):
        ...
        # Fix sampling RNG so the eval samples are reproducible and *paired* across
        # baseline vs fix at the same seed (matched randn noise → only model differs).
        # Save/restore global RNG state so training's RNG stream is untouched.
