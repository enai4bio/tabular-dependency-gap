#!/usr/bin/env bash
# 059 Task 1: bank downward capacity sweep, QUEUE B of 3. Disjoint from _A.sh/_C.sh.
set -uo pipefail
cd "$(dirname "$0")"
...
run bank "bank_cap0.5x_s3" 3 "$CHALF"
run bank "bank_cap0.5x_s4" 4 "$CHALF"
run bank "bank_cap0.25x_s0" 0 "$CQUARTER"
run bank "bank_cap0.25x_s1" 1 "$CQUARTER"
run bank "bank_cap0.125x_s2" 2 "$CEIGHTH"

echo "[$(date '+%F %T')] QUEUE 059-B COMPLETE" | tee -a "$LOG"
