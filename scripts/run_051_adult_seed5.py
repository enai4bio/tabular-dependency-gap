"""051 P4 (free re-eval): adult diagnostic + 2a to 5 seeds (5 models already exist).
FF/TabbyFlow/Oracle: full/marg/dep/dep_cross + minority F1, and 2a moment gap. 5 seeds.
Outputs: step_adult_diag_seed5_051.csv, step_adult_order_seed5_051.csv (+_long).
"""
import os, sys
...
def main():
    print("=" * 60); print("051 adult diagnostic + 2a -> 5 seeds (free re-eval)"); print("=" * 60)
    ...
    diag, order = [], []
    for seed in SEEDS:
        syn = pd.read_csv(f"ef_vfm/result/adult/adult_base_s{seed}/8000/samples.csv"); n = len(syn)
        ...
        for name, sdf in variants.items():
            ...
            diag.append(dict(dataset="adult", variant=name, seed=seed, full=m["xgb_c2st"], marg=m["marg_c2st"],
                             dep=m["dep_c2st"], dep_cross=dc, mle_f1=m["mle_f1"], mle_recall=m["mle_recall"]))
            mg, cg, npr = moment_gap(test, sdf, info)
            order.append(dict(dataset="adult", variant=name, seed=seed, mean_gap=mg, corr_gap=cg, n_pairs=npr))
    ...
    for rows, met, tag in [(diag, ["full","marg","dep","dep_cross","mle_f1","mle_recall"], "diag"),
        ...
        L = pd.DataFrame(rows); L.to_csv(f"step_adult_{tag}_seed5_051_long.csv", index=False)
        ...
        g.to_csv(f"step_adult_{tag}_seed5_051.csv", index=False)
    print("\n=== adult diag 5seed ==="); print(pd.read_csv("step_adult_diag_seed5_051.csv")[["variant","dep_mean","dep_std","dep_cross_mean","mle_f1_mean"]].to_string(index=False))
    print("=== adult 2a 5seed ==="); print(pd.read_csv("step_adult_order_seed5_051.csv")[["variant","mean_gap_mean","mean_gap_std","corr_gap_mean"]].to_string(index=False))
    print("\nDONE.")
