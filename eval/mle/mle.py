...
def _evaluate_multi_classification(train, test, info, val=None, seed=42):
    np.random.seed(seed)
    ...
    for model_spec in classifiers:
        ...
        model_kwargs = {**model_spec.get('kwargs', dict()), 'random_state': [seed]}
...
@ignore_warnings(category=ConvergenceWarning)
def _evaluate_binary_classification(train, test, info, val=None, seed=42):
    np.random.seed(seed)
    x_trains, y_trains, x_valid, y_valid, x_test, y_test, classifiers = prepare_ml_problem(train, test, info, val=val)
    ...
    for model_spec in classifiers:
        ...
        def _df(dataframe):
            return {
                ...
                "weighted_f1": dataframe.weighted_f1.values[0],
                "accuracy": dataframe.accuracy.values[0],
                "minority_recall": dataframe.recall.values[0],
                "minority_precision": dataframe.precision.values[0],
            }
...
@ignore_warnings(category=ConvergenceWarning)
def _evaluate_regression(train, test, info, val=None, seed=42):
    np.random.seed(seed)
